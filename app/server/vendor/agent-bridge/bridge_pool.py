        ...
        with _profile_env(profile):
            ...
            def stream_callback(delta: str) -> None:
                with self._lock:
                    ...
                    # `events` list used by tool.started/tool.completed, so that
                    # when the model interleaves narration and tool calls
                    # ("text → tool → more text") the consumer sees the true
                    # relative ordering — otherwise all tool events would be
                    # processed before the aggregated delta and a word could be
                    # visibly split across the tool boundary.
            ...
            try:
                ...
                with session.lock:
                    ...
                    elif result.get("failed") and not final_response:
                        # Local patch: surface engine failure to pollers instead of
                        # reporting a normal completion with empty output. Any
                        # usable content (final_response falls back to deltas)
                        # still completes normally; the consumer appends a
                        # warning based on result.failed.
                        record.status = "error"
                        err_val = result.get("error")
                        if isinstance(err_val, str) and err_val.strip():
                            record.error = err_val
                        elif err_val:
                            try:
                                record.error = json.dumps(err_val, ensure_ascii=False, default=str)
                            except Exception:
                                record.error = str(err_val)
                        else:
                            record.error = "agent run failed"
                    else:
