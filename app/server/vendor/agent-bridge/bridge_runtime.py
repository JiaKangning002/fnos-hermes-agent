...
def _candidate_agent_roots(raw: str | None = None) -> list[Path]:
    ...
    # home 默认安装目录优先于 cwd 派生候选，避免从含 run_agent.py 的目录手动启动时被劫持
    candidates.extend([
    ...
    # venv/系统 site-packages（pip 平铺安装时 run_agent.py 位于其顶层）
    try:
...
def _site_packages_paths() -> set[str]:
    """当前解释器的 site-packages 目录集合（用于标识宽松探测的命中来源）。"""
    paths: set[str] = set()
    try:
        paths.add(str(Path(sysconfig.get_paths()["purelib"]).resolve()))
    except Exception:
        pass
    try:
        for entry in site.getsitepackages():
            paths.add(str(Path(entry).resolve()))
    except Exception:
        pass
    return paths
...
def _set_path_env(agent_root: str | None = None, hermes_home: str | None = None) -> None:
    ...
    if resolved_root is not None:
        if not agent_root and str(resolved_root) in _site_packages_paths():
            _bridge_log("agent_root_site_packages", {"path": str(resolved_root)})
...
def _ensure_agent_imports() -> None:
    ...
    if root is not None:
        ...
        _bridge_log("agent_root_find_spec_fallback", {"origin": str(getattr(spec, "origin", "") or "")})
